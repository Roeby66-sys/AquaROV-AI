...
from PySide6.QtWidgets import QGridLayout, QLabel, QMainWindow, QSizePolicy, QVBoxLayout, QWidget

from dto import (
    Alert, AppConfig, CameraChannel, DetectionSet, MissionState,
    RecordingState, SystemMetrics,
)
from camera_manager import CameraManager
from inference_worker import (
    InferenceBackend, InferenceFrame, InferenceResult,
    InferenceWorker, NullInferenceBackend,
)
    ...
    def __init__(
        ...
        *,
        ...
        self._bridge.detection_ready.connect(
            self._on_detection_ready, Qt.ConnectionType.QueuedConnection
        )
        self._bridge.inference_error.connect(
            self._on_inference_error, Qt.ConnectionType.QueuedConnection
        )
        self._bridge.camera_error.connect(
            self._on_camera_error, Qt.ConnectionType.QueuedConnection
        )
